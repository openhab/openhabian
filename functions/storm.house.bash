...
setup_pv_config() {
  ...
  if [[ -n "$INTERACTIVE" ]]; then
    if [[ -z "${1:-$invertertype}" ]]; then
        if ! invertertype="$(whiptail --title "Wechselrichter Auswahl" --cancel-button Cancel --ok-button Select --menu "\\nWählen Sie den Wechselrichtertyp aus" 17 100 8 "sunspec" "SunSpec kompatibel (ungetestet)" "kostal" "Kostal Plenticore" "sungrow" "Sungrow SH RT" "solaredge" "SolarEdge SE (experimental)" "fronius" "Fronius Symo (experimental)" "senec" "Senec Home (ungetestet)" "victron" "Victron mit Gateway (experimental)" "solax" "Solax X1/X3 (experimental)" "pvmanuell" "manuelle Konfiguration" 3>&1 1>&2 2>&3)"; then unset invertertype; return 1; fi
    fi
