        ...
        if [ "$1" == "Zulu8-64" ] || [ "$1" == "Zulu11-64" ]; then
          if is_aarch64 || is_x86_64 && [ "$(getconf LONG_BIT)" == "64" ]; then
            if is_x86_64; then
              ...
              if [ "$1" == "Zulu8-64" ]; then
                if java_zulu_update_available Zulu8-64; then
                  ...
                  if ! [ -x "$(command -v java)" ] && ! [ -d /opt/jdk/* ]; then fetch_zulu Zulu8-64; fi
                  java_zulu_install
                fi
              elif [ "$1" == "Zulu11-64" ]; then
                if java_zulu_update_available Zulu11-64; then
                  echo -n "$(timestamp) [openHABian] Installing Java Zulu 11 64-Bit OpenJDK... "
                  if ! [ -x "$(command -v java)" ] && ! [ -d /opt/jdk/* ]; then fetch_zulu Zulu11-64; fi
                  java_zulu_install
                fi
              fi
            fi
            ...
            if [ -n "$INTERACTIVE" ]; then
              ...
              if java_zulu_update_available Zulu8-32; then
                ...
                if ! [ -x "$(command -v java)" ] && ! [ -d /opt/jdk/* ]; then fetch_zulu Zulu8-32; fi
                java_zulu_install
              fi
            fi
          fi
          ...
          if [ "$1" == "Zulu11-32" ]; then
            if java_zulu_update_available Zulu11-32; then
              ...
              if ! [ -x "$(command -v java)" ] && ! [ -d /opt/jdk/* ]; then fetch_zulu Zulu11-32; fi
              java_zulu_install
            fi
...
fetch_zulu(){
  ...
  if [ "$1" == "Zulu8-32" ]; then
    ...
    echo -n "[DEBUG] Invalid argument to function fetch_zulu()"
    exit 1
  fi
