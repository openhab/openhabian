...
amanda_setup() {
  ...
  introText="This will setup a backup mechanism to allow for saving your openHAB setup and modifications to either USB attached or Amazon cloud storage.\\n\\nYou can add your own files/directories to be backed up, and you can store and create clones of your openHABian SD card to have an pre-prepared replacement in case of card failures. Warning, running this setup will overwrite any previous Amanda backup configurations.\\n\\nWould you like to begin setup?"
  ...
  successText="Setup was successful.\\n\\nAmanda backup tool is now taking backups at 01:00. For further readings, start at http://wiki.zmanda.com/index.php/User_documentation."
  ...
  if (whiptail --title "Backup using locally attached storage?" --yes-button "Yes" --no-button "No" --yesno "Would you like to setup a backup mechanism based on locally attached or NAS mounted storage?" 8 80); then
  ...
  elif (whiptail --title "Backup using Amazon AWS?" --yes-button "Yes" --no-button "No"  --defaultno --yesno "Would you like to setup a backup mechanism based on Amazon Web Services?\\n\\nYou can get 5 GB of S3 cloud storage for free on https://aws.amazon.com/. For hints see http://markelov.org/wiki/index.php?title=Backup_with_Amanda:_tape,_NAS,_Amazon_S3#Amazon_S3\\n\\nPlease setup your S3 bucket on Amazon Web Services NOW if you have not done so. Remember the name has to be unique in AWS namespace." 15 80); then
    config="openhab-AWS"
    ...
    whiptail --title "Operation Canceled!" --msgbox "No backup options were selected and as such none were configured!" 7 80
    return 0
  fi
