...
system_upgrade() {
  ...
  if ! apt-get upgrade --yes --force-confnew -o DPkg::Lock::Timeout="$APTTIMEOUT" -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold"; then echo "FAILED"; return 1; fi
...
locale_setting() {
  ...
  if [[ -n $INTERACTIVE ]]; then
    whiptail --title "Change locale" --msgbox "For the locale change to take effect, please reboot your system now." 11 80
  fi
}
...
prepare_serial_port() {
  ...
  "1"  "Disable serial console to give room for HATs"      $optionOne \
  "2"  "move Bluetooth to mini UART" $optionTwo \
