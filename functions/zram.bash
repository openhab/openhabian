...
init_zram_mounts() {
  ...
  if [[ $1 == "install" ]] && ! [[ -f /etc/ztab ]]; then
    ...
    if ! sed -e "s|%STORAGE=|${storageDir}|g" "$zramInstallLocation"/openhabian-zram/zram-config.service > /etc/systemd/system/zram-config.service; then echo "FAILED (install service)"; return 1; fi
    if ! sed -e "s|%STORAGE=|${storageDir}|g" "$zramInstallLocation"/openhabian-zram/zramsync.service > /etc/systemd/system/zramsync.service; then echo "FAILED (install service)"; return 1; fi
