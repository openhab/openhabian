...
versioning_setup() {
  ...
  local user repositiory token logPrefix email
  ...
  email="$github_email"
  # shellcheck disable=SC2154
  repositiory="$github_repositiory"
  # shellcheck disable=SC2154
  token="$github_token"
  logPrefix="$(timestamp) [openHABian]"

  if [[ -n $UNATTENDED ]] 
  then
    if [[ -z $github_token ]]
    then
      echo "$logPrefix Github Versioning setup... CANCELED (Valid configuration not provided)"
      return 0
    fi
  fi
  ...
  echo -n "$logPrefix Beginning the Github Repositiory Setup... "
  if [[ -n $INTERACTIVE ]]
  then 
  ...
  echo -n "$logPrefix Gathering Credentials for Github..."
    ...
    if [ -z "$repositiory" ] 
    then
      if ! repositiory="$(whiptail --title "Versioning Github" --inputbox "\\nGithub Repository?" 8 39 --title "$questionText Repository" 3>&1 1>&2 2>&3)"; then
        echo "CANCELED REPO"; return 0
      fi
    fi
  ...
  repourl="https://$user:$token@github.com/$user/$repositiory.git"
  echo -n "$logPrefix Check if $repositiory Exists and credentials valid..."
  ...
  echo -n "$logPrefix Check if $repositiory is set to Private..."
  if curl -s --head --request GET https://github.com/"$user"/"$repositiory" | grep "HTTP/2 404" > /dev/null
  then
    echo " OK"
  else
    echo " FAIL"
    if [[ -n $INTERACTIVE ]]
    then
      whiptail --title "Openhabian Versioning" --msgbox "Please Set Repository [$repositiory] to Private on Github and try again" 8 78
      return 0
    fi
  fi
 
  apiurl="https://api.github.com/repos/$user/$repositiory/contents/openhabian_config_versioning"
  echo -n "$logPrefix Check online Repository [$repositiory] contains previous install..."
  ...
  then
    ...
    echo "$logPrefix This file is intentionly left empty" > /openhabian_config_versioning
  ...
  then 
    ...
    exsisting_userename=$(git -c "/" config user.name)
    echo "$logPrefix Git name already set [$exsisting_userename]... SKIPPED"
  fi
  ...
  then 
    ...
    exsisting_email=$(git -c "/" config user.email)
    echo "$logPrefix Git email already set [$exsisting_email]... SKIPPED"
  fi

  git -C "/" config versioning.repositiory "$repositiory"
...
versioning_uninstall() {
  ...
  if (whiptail --title "Versioning Github" --yes-button "Continue" --no-button "Cancel" --yesno "Found Existing Repositiory\\n\\n$currentrepo\\n\\nUninstall Versioning are you sure you want to continue" 14 80)
  then
    echo "OK"
  else
    return 0
...
versioning_commit() {
  ...
  if [ -d "/.git" ]
  then 
    echo "[openHABian] Start Repositiory commit... "
    ...
    echo "[openHABian] No Repositiory Found to commit... "
    return 0
  fi
...
versioning_tokenupdate() {
  ...
  local newtoken repositiory user newrepourl

  newtoken="$(whiptail --title "Versioning Github" --inputbox "\\nGithub Token?" 8 39 --title "Enter new Token" 3>&1 1>&2 2>&3)"
  repositiory=$(git -C "/" config versioning.repositiory)
  user=$(git -C "/" config user.user)
  newrepourl="https://$user:$newtoken@github.com/$user/$repositiory.git"
  echo "$newrepourl"
