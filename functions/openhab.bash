...
openhab_setup() {
  ...
  if [[ -z $OFFLINE ]]; then
    ...
    echo "$repo" > /etc/apt/sources.list.d/${ohname}.list

    echo -n "$(timestamp) [openHABian] Installing selected openHAB version... "
    if ! cond_redirect apt-get update; then echo "FAILED (update apt lists)"; return 1; fi
    openhabVersion="$(apt-cache madison ${ohname} | head -n 1 | cut -d'|' -f2 | xargs)"
    if cond_redirect apt-get install --allow-downgrades --yes "${ohname}=${openhabVersion}" "${ohname}-addons=${openhabVersion}"; then echo "OK"; else echo "FAILED"; return 1; fi
  else
    echo -n "$(timestamp) [openHABian] Installing cached openHAB version... "
    if cond_redirect apt-get install --yes ${ohname} ${ohname}-addons; then echo "OK"; else echo "FAILED"; return 1; fi
  fi
  ...
  if cond_redirect systemctl enable ${ohname}.service; then echo "OK"; else echo "FAILED (enable service)"; return 1; fi
