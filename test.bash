...
if [ "$1" == "docker-full" ]; then
    ...
    cond_redirect docker build --tag openhabian/rpi-openhabian .
    cond_redirect docker run -it openhabian/rpi-openhabian bash -c 'bats -r -f "unit-." .'
    cond_redirect docker run --name "install-test" --privileged -d openhabian/rpi-openhabian
    cond_redirect docker exec -it install-test bash -c "./build.bash local-test && mv ~/.profile ~/.bash_profil && /etc/rc.local"
    cond_redirect docker commit install-test openhabian-installed
